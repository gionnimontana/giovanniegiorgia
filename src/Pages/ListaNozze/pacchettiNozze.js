const packages = [
	{
		id: 'V1',
		img: 'https://www.traveldailymedia.com/assets/2018/11/United-Airlines-Boeing-787-9-Dreamliner-1024x576.jpg',
		title: 'Voli aerei',
		text: 'Milano - San Francisco, Los Angeles - Papeete, e volo di ritorno, che lasciamo per ultimo, dovesse mancare ce ne faremo una ragione :)',
		purchased: 0
	},
	{
		id: 'V2',
		img: 'https://www.flightcentre.ca/blog/wp-content/uploads/2016/03/iStock-California-road-trip.jpg',
		title: 'Noleggio auto e carburante',
		text: "Visto i recenti aumenti ci serve una sezione dedicata di donazioni, ricordando sempre che 'Se un uomo apre la portiera dell’auto alla moglie, o è nuova l’auto o è nuova la moglie'",
		purchased: 0
	},
	{
		id: 'V3',
		img: 'https://qtxasset.com/styles/breakpoint_sm_default_480px_w/s3/fiercepharma/1610037461/GettyImages-1136437406.jpg/GettyImages-1136437406.jpg?VersionId=lKL0To.RG2Hc7qtD3t1UWLR6idJOG5ng&itok=FE3sBy4U',
		title: 'Big city life - Metropoli Americane',
		text: 'San Francisco, Los Angeles, Las Vegas. il 10% di quello che donate qui li spenderemo in fast foods, un altro 10% in acquisti stupidi, il resto finiranno tutti su un tavolo da BlackJack!',
		purchased: 0
	},
	{
		id: 'V8',
		img: 'https://pix10.agoda.net/geo/city/3962/1_3962_02.jpg?ca=6&ce=1&s=1920x822',
		title: 'Giro al parchetto',
		text: "Sequoia National Park (dove ci sono gli alberi piu' grandi del mondo), Monument Valley, Grand Canyon, Page Canyon, Bryce Canyon. Li acchiapperemo tutti!",
		purchased: 0
	},
	{
		id: 'V13',
		img: 'https://www.downunderendeavours.com/wp-content/uploads/2013/11/Moorea-900x600-lodge.jpg',
		title: 'Una destinazione da sogno',
		text: "Papeete, Mo'orea, Bora Bora. Le sognamo da una vita e non vediamo l'ora di nuotarci intorno!",
		purchased: 0
	},
	{
		id: 'V14',
		img: '/img/bagno-squali-min.jpeg',
		title: 'Cose che fanno paura a Giorgia',
		text: 'Un bagno con li squali in polinesia, un esperienza hippie a San Francisco, di sicuro le idee non ci mancano, chissà se ci donerete anche il budget necessario :) ',
		purchased: 0
	},
	{
		id: 'V16',
		img: 'https://immagini.quotidiano.net/?url=http%3A%2F%2Fp1014p.quotidiano.net%3A80%2Fpolopoly_fs%2F1.7220253.1643190294%21%2FhttpImage%2Fimage.jpg_gen%2Fderivatives%2Fwidescreen%2Fimage.jpg&w=660&h=368',
		title: 'TAMPONI',
		text: "Perchè dove se il tampone non ce l'hai?",
		purchased: 0
	}
]

export default packages
