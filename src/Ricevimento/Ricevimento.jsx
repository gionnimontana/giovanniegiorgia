...
const Ricevimento = () => {
    ...
    const onConfirm = async () => {
        ...
        if (ciSaro) {
            const userPromises = confirmed.map(el => {
                const payload = {name: el.name, confirmation: ciSaro, menu_options: el.selected, clusterId}
                return startExecuteInsert_users(payload, clusterId)
            })
            await Promise.all(userPromises)
        } else {
            await startExecuteInsert_users({ name: `${name} ${surname}`, confirmation: false, clusterId}, clusterId)
        }
        setLoading(false)
    }
    ...
    return (
        <div>
            {canEdit && <div className="info_container">
            ...
            {!canEdit && (
                <div className="info_container">
                    <div className="info_uppertitle">{name.toUpperCase()}, 
